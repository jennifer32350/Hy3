...
- `cases/v0.2-phase-a.json`：36 个机器可读问题，基础统计、复合分析、趋势、异常、质量和
  可视化各 6 个。
- `cases/v0.2-release.json`：直接通过正式 Workflow Schema、质量层和执行器运行的阶段 E
  发布回归案例，覆盖 `distribution`、`pivot_table` 和 `derived_metric`。
- `baselines/v0.1.json`：当前 v0.1 单步 Schema 和确定性执行器的冻结结果。
- `run_evals.py`：固定白名单的离线 Oracle，验证案例中的 49 个精确数值断言、数据/清单
  SHA-256、v0.1 兼容执行结果，以及 v0.2 所需操作覆盖率。
...
v0.2 离线摘要另外报告 `v02_offline_operation_coverage_rate`：它只检查 36 个案例声明的
所需分析/渲染操作是否都属于正式版实现范围，并与 49 个本地 Oracle 数值断言一同运行。
该指标不是 Planner 成功率，也不是端到端成功率；对应 live 指标仍为 `null`。
发布回归案例则会实际执行生产代码，并对分位数、分箱计数、完整 Pivot 网格、填充值和
派生利润率执行精确断言。

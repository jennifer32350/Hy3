...
选择图表类型、字段和受限 `AnalysisWorkflow`；本地校验器、质量层与工作流执行器重新计算
数据，Renderer 只读取绑定到 `evidence_id` 的确定性 records。
...
真实 Cursor/CodeBuddy 图像展示和录屏依赖用户客户端与授权 Hy3 endpoint，属于阶段 G 人工
验收，离线测试不能替代。
